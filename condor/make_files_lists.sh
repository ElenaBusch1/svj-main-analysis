...
find $eosPath -name "user.ebusch*" -type f -size +900M | cut -d "/" -f 10 > v12p5_large_files.txt
echo""

echo "Small files to run on condor: "
find $eosPath -name "user.ebusch*" -type f -size -900M | cut -d "/" -f 10
find $eosPath -name "user.ebusch*" -type f -size -900M | cut -d "/" -f 10 > v12p5_condor_files.txt
